...
class ArchiveSynthesizer:
        ...
        for endian in ("<", ">"):
            for count_off in (4, 8, 0, 12):
                if count_off + 4 > total_len:
                    continue
                cand_count = schema.unpack(f"{endian}I", data[count_off : count_off + 4])[0]
    ...
    def _validate_toc(
        ...
        for i in range(count):
            ...
            off, sz = schema.unpack(f"{endian}II", data[e_off : e_off + 8])
    ...
    def _probe_offset_table(cls, data: bytes, endian: str) -> List[Tuple[int, int]]:
        ...
        while pos + 4 <= total_len:
            val = schema.unpack(f"{endian}I", data[pos : pos + 4])[0]
            if not offsets:
...
class SynthesizedArchive:
    ...
    def _unpack(self):
        ...
        for i in range(count):
            ...
            off, size = schema.unpack_from(f"{{self.ENDIAN}}II", self.data, e_off)
    ...
    def pack(self) -> bytes:
        ...
        for f in self.files:
            toc += schema.pack(f"{{self.ENDIAN}}II", cur_off + len(payload), len(f))
            payload += f
            # 16-byte alignment
